...
public class ListaObj <T> {
  ...
  public RelatorioDTO pesquisaBinaria(ListaObj lista, LocalDateTime dataAlvo){
    int esquerda = 0;
    int direita = nroElem - 1;

    while (esquerda <= direita) {
      int meio = esquerda + (direita - esquerda) / 2;
      RelatorioDTO relatorioNoMeio = (RelatorioDTO) vetor[meio];
  ...
  public static void gravaArquivoCsv(ListaObj<RelatorioDTO> lista, String nomeArq) {
    FileWriter arq = null;
    ...
    try {
      for (int i = 0; i < lista.getTamanho(); i++) {
        ...
        RelatorioDTO relatorio = lista.getElemento(i);
